...
import { ChartNoAxesColumnIncreasing, CircleDollarSign, Leaf, Utensils } from "lucide-react";
...
export function DishSummaryCard({ group }: { group: DishGroup }) {
  ...
  return (
    <section className="flex min-h-[99px] gap-3 rounded-xl border border-line bg-card p-2.5 shadow-[0_1px_4px_rgba(23,27,32,0.05)]">
      <DishVisual
        category={representative?.canonical_category ?? null}
        name={group.displayName}
        className="h-[78px] w-[82px] rounded-lg"
        showLabel
      />
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-3">
          ...
          {representative ? <SaveButton kind="dish" item={representative} compact /> : null}
        </div>
